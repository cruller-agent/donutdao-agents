...
function determineFundingStrategy(balances) {
  ...
  if (balances.base?.eth > parseEther('0.0005')) {
  ...
  if (balances.ethereum?.eth > parseEther('0.0005')) {
...
async function autoSetup(privateKey, castText = 'gm! this account was created autonomously by an AI agent', options = {}) {
  ...
  if (!strategy.ready) {
    ...
    // Handle Base ETH -> Bridge to Optimism
    if (strategy.source === 'base' && strategy.sourceAsset === 'eth') {
      const baseProvider = new JsonRpcProvider(CHAINS.base.rpc);
      const baseWallet = new Wallet(privateKey, baseProvider);

      const ethBalance = await baseProvider.getBalance(baseWallet.address);
      // Bridge 80% to Optimism, keep some for gas
      const bridgeAmount = ethBalance * 80n / 100n;

      console.log('Bridging', formatEther(bridgeAmount), 'ETH from Base to Optimism...');
      await bridgeViaAcross(baseWallet, 'base', 'optimism', bridgeAmount);
      console.log('Bridge initiated (may take 1-10 minutes)');
      ...
      for (let i = 0; i < 60; i++) { // Wait up to 10 minutes
        await new Promise(r => setTimeout(r, 10000));
        opBalance = await opProvider.getBalance(baseWallet.address);
    ...
    // Handle Ethereum ETH -> Bridge to Optimism
    if (strategy.source === 'ethereum' && strategy.sourceAsset === 'eth') {
      const ethProvider = new JsonRpcProvider(CHAINS.ethereum.rpc);
      const ethWallet = new Wallet(privateKey, ethProvider);

      const ethBalance = await ethProvider.getBalance(ethWallet.address);
      const bridgeAmount = ethBalance * 90n / 100n; // Bridge 90%

      console.log('Bridging', formatEther(bridgeAmount), 'ETH from Ethereum to Optimism...');
      await bridgeViaAcross(ethWallet, 'ethereum', 'optimism', bridgeAmount);
      console.log('Bridge initiated (may take 1-10 minutes)');
      ...
      for (let i = 0; i < 60; i++) {
        await new Promise(r => setTimeout(r, 10000));
        opBalance = await opProvider.getBalance(ethWallet.address);
        if (opBalance > parseEther('0.0003')) {
          console.log('Bridge complete! Optimism balance:', formatEther(opBalance));
          break;
        }
